...
class TMDBClient:
    """
    TMDb API client with session reuse and dual-layer caching
    (in-memory + Django cache) for optimized performance.1-=
    """

    BASE = "https://api.themoviedb.org/3"
    IMAGE_BASE = "https://image.tmdb.org/t/p/"

    def __init__(self, bearer_token: str | None = None, language: str = "pt-BR"):
        self.language = language
        self.headers = {"Authorization": bearer_token or settings.TMDB_BEARER}
    ...
    def movie_details(self, tmdb_id: int) -> Dict[str, Any]:
        """Retorna detalhes do filme + vídeos + provedores + elenco/direção, com URLs completas de imagem."""
        details = self._cached_request(
            f"/movie/{tmdb_id}",
        ...
        providers_data = self._cached_request(
        ...
        credits_data = self._cached_request(
